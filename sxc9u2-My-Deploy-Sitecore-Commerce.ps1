#Requires -Version 3
param(
    [string]$SiteInstance = "sc9u2",	
	[string]$SiteHostHeaderName = "sxa.storefront.com",	
	[string]$SqlDbPrefix = "$($SiteInstance)",
...
$params = @{
		...
		SiteName = "$($SiteInstance).sc"
		SiteHostHeaderName = $SiteHostHeaderName 
		InstallDir = "$($Env:SYSTEMDRIVE)\inetpub\wwwroot\$($SiteInstance).sc"
		XConnectInstallDir = "$($Env:SYSTEMDRIVE)\inetpub\wwwroot\$($SiteInstance).xconnect"
		CertificateName = "$($SiteInstance).sc"
		CommerceServicesDbServer = $($Env:COMPUTERNAME)    
		CommerceServicesDbName = "$($SiteInstance)_SharedEnvironments"
		CommerceServicesGlobalDbName = "$($SiteInstance)_Global"
		CommerceServicesPostfix = "$($SiteInstance)"		
		...
		CommerceEngineCertificatePath = Resolve-Path -Path "..\assets\sc9u2.engineconnect.cer"		
